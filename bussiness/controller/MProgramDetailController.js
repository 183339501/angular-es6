 ...
 */
 export default class MProgramDetailController {
    ...
    initData () {
        var promise = this.httpService.sendRequest("get","/v1/mPrograms/"+this.pid);
        promise.then(data=>{
            this.program = data;
        },err=>{
            alert("查询失败");
    ...
    save(e) {
        ...
        var promise = this.httpService.sendRequest("put","v1/mPrograms",this.program);
        promise.then(data=>{
            target.button('reset');
            alert("保存成功")
        },err=>{
...
MProgramDetailController.$inject = ["$scope","HttpService","$stateParams"];